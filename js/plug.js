...
/*        $('.searchInput02').focus(function () {
        ...
        $('.searchInput02Wraper').hover(
            function(){
                ...
                $('.searchGroupMenu').show();
                $('.searchGroupMenu li').click(function () {
                	var basepath = $('#dynamicSearch').attr('action');
                	basepath = basepath.substring(0,basepath.lastIndexOf("/")+1);
                	if ($(this).text() == '领域专家'){
                		basepath += 'searchexpertlist.do';
                		$('#dynamicSearch').attr({'action':basepath});
                	} else if($(this).text() == '项目需求'){
                		basepath += 'searchprojectlist.do';
                		$('#dynamicSearch').attr({'action':basepath});
                	}
    ...
    $.fn.autoCompleteSreachBox = function () {
        ...
        $.ajax({
            ...
            success: function(response, status, xhr) {
                ...
                //    $Input.next('.errorTip').find('.errorText').text('未注册');
                //    return false;
                //}
                dataArr = response;
                //执行初始化显示下拉选项函数
...
//上38
    ...
    //项目消息创建tab插件
    $.fn.projectMessageTabs = function () {
        ...
        $(this).each(function () {
            ...
            $(this).children().children(".tabBtn").click(function () {
                ...
                $(this).parent().find(".tabBtn").css({"color": "#2c84cc"}).removeClass('active');
                $(this).css({"color": "black"}).addClass('active');
    ...
    //设置消息提示插件
    $.fn.setiingTip = function (obj) {
                                   function Tip(show,hide,time){
                                       show();
                                   ...
                                   Tip(
                                   function show(){
                                       $('body').append($("<div id='setiingTip' class='setiingTip'>"+obj.text+"</div>"));
                                               if(obj.type==1){
                                                    $('#setiingTip').removeClass('error').addClass('success').fadeIn();
                                                }else{
                                                    $('#setiingTip').removeClass('success').addClass('error').fadeIn();
                                                }
                                       },
                                   function hide(){
                                       $('#setiingTip').fadeOut(function(){$(this).remove()});
                                       },
                                   4000)
    };
    ...
    //session过期插件
    $.fn.sessionExpiredDia = function () {
            $('body').append('<div class="sessionExpiredDia" id="sessionExpiredDia">\
                <img src="images/sessionExpiredDiaColseBtn.png" class="sessionExpiredDiaColseBtn"  alt="">\
                <h1 class="title">提示</h1>\
                <div class="content">由于您长时间未操作，会话已失效，请重新登录！</div>\
                <div class="operation clearfix">\
                <button class="btn btn-default btn-block" id="sessionExpiredDiaCaBtn"type="button">取消</button>\
                <button class="btn btn-primary btn-block" id="sessionExpiredDiaSiBtn" type="button">重新登录</button>\
                </div>\
                </div>');
            $('#sessionExpiredDia').fadeIn();

        $('body').delegate('#sessionExpiredDiaSiBtn','click', function() {
            $('#sessionExpiredDia').fadeOut(function(){
                $('#signBtn').click();
                $('input[name= "signInDialogProjectId"]').val($('input[name= "projectId"]').val());
                $(this).remove();
            });
        });
        $('body').delegate('#sessionExpiredDiaCaBtn,.sessionExpiredDiaColseBtn', 'click', function() {
            $('#sessionExpiredDia').fadeOut(function(){
                $(this).remove();
            });
        });
